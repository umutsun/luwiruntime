...
export function buildFunctionLibrary(registry: RedisFunctionRegistry): RedisFunctionLibrary {
  const source = [
    ...
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  if message.retryOf ~= nil and (type(message.retryOf) ~= 'string' or message.retryOf == '') then",
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    ...
    "  if type(message.retryOf) == 'string' then stored.retryOf = message.retryOf end",
    '  local inbox_item = {',
    ...
    "    payload={messageId=message.id, kind=message.kind, state='queued', sourceSessionId=message.sourceSessionId, targetSessionId=message.targetSessionId, selectionReason=message.selectionReason, retryOf=stored.retryOf}",
    '  }',
    '  local event_json = cjson.encode(event)',
    ...
    "  if stored.retryOf then redis.call('HSET', keys[1], 'retryOf', stored.retryOf) end",
    ...
    '  if raw.retryOf then result.retryOf = raw.retryOf end',
    ...
    "      return cjson.encode({status='error', code='RESPONDER_SESSION_MISMATCH'})",
    '    end',
    '    -- A terminal target may still fail its own in-flight messages: the daemon',
    '    -- does so for it the moment the session ends (TARGET_SESSION_LOST).',
    "    if current ~= target_state and target_state ~= 'failed' and (responder[2] == 'completed' or responder[2] == 'disconnected') then",
    "      return cjson.encode({status='error', code='RESPONDER_SESSION_MISMATCH'})",
    '    end',
    '  end',
    ...
    'end',
    // Per-project autopilot, goals and tasks (ADR 0035).
    //
    // Every rule about which move is legal lives in `@luwi/runtime`; what
    // happens here is only the part that has to be atomic — a compare-and-set
    // on the record's own `version`, the index writes that belong with it, and
    // the event. Every key is declared by the caller; nothing is derived.
    'local function cas_check(key, expected_version, incoming, id_field)',
    "  if type(incoming) ~= 'table' or type(incoming.version) ~= 'number' or type(incoming[id_field]) ~= 'string' or incoming[id_field] == '' then",
    "    return 'REDIS_ARGUMENT_INVALID', nil, nil",
    '  end',
    "  local exists = key_type(key) ~= 'none'",
    '  if expected_version == 0 then',
    "    if exists then return 'VERSION_CONFLICT', nil, nil end",
    "    if incoming.version ~= 1 then return 'REDIS_ARGUMENT_INVALID', nil, nil end",
    '    return nil, nil, nil',
    '  end',
    "  if not exists then return 'VERSION_CONFLICT', nil, nil end",
    "  local stored_json = redis.call('HGET', key, 'json')",
    '  local stored_ok, stored = pcall(cjson.decode, stored_json or "")',
    "  if not stored_ok or type(stored) ~= 'table' then return 'REDIS_STATE_INVALID', nil, nil end",
    "  if stored[id_field] ~= incoming[id_field] then return 'REDIS_STATE_INVALID', stored, stored_json end",
    "  if tonumber(stored.version) ~= expected_version then return 'VERSION_CONFLICT', stored, stored_json end",
    "  if incoming.version ~= expected_version + 1 then return 'REDIS_ARGUMENT_INVALID', stored, stored_json end",
    '  return nil, stored, stored_json',
    'end',
    'local function autopilot_put(keys, args)',
    '  if #keys ~= 4 or #args ~= 3 then',
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  if not type_is(keys[1], 'hash') or not type_is(keys[2], 'set') or not type_is(keys[3], 'stream') or not type_is(keys[4], 'stream') then",
    ...
    '  local record_ok, record = pcall(cjson.decode, args[1])',
    '  local event_ok, event = pcall(cjson.decode, args[2])',
    '  local expected = tonumber(args[3])',
    "  if not record_ok or not event_ok or type(event) ~= 'table' or not expected then",
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  local failure, stored, stored_json = cas_check(keys[1], expected, record, 'projectId')",
    "  if failure == 'VERSION_CONFLICT' then return cjson.encode({status='version_conflict', json=stored_json}) end",
    "  if failure then return cjson.encode({status='error', code=failure}) end",
    '  if not stream_appendable(keys[3]) or not stream_appendable(keys[4]) then',
    "    return cjson.encode({status='error', code='REDIS_STATE_INVALID'})",
    '  end',
    "  redis.call('HSET', keys[1], 'id', record.projectId, 'json', args[1])",
    "  redis.call('SADD', keys[2], record.projectId)",
    '  local streams = control_append_event(keys[3], keys[4], args[2])',
    "  return cjson.encode({status='written', json=args[1], globalStreamId=streams.globalStreamId, projectStreamId=streams.projectStreamId})",
    'end',
    'local function goal_write(keys, args)',
    '  if #keys ~= 5 or #args ~= 5 then',
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  if not type_is(keys[1], 'hash') or not type_is(keys[2], 'zset') or not type_is(keys[3], 'zset') or not type_is(keys[4], 'stream') or not type_is(keys[5], 'stream') then",
    "    return cjson.encode({status='error', code='REDIS_STATE_INVALID'})",
    '  end',
    '  local goal_ok, goal = pcall(cjson.decode, args[1])',
    '  local expected = tonumber(args[3])',
    '  local created_ms = tonumber(args[4])',
    '  local retrospective_ms = tonumber(args[5])',
    '  if not goal_ok or not expected or not created_ms then',
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  local failure, stored, stored_json = cas_check(keys[1], expected, goal, 'id')",
    "  if failure == 'VERSION_CONFLICT' then return cjson.encode({status='version_conflict', json=stored_json}) end",
    "  if failure then return cjson.encode({status='error', code=failure}) end",
    "  local has_event = args[2] ~= ''",
    '  if has_event then',
    '    local event_ok, event = pcall(cjson.decode, args[2])',
    "    if not event_ok or type(event) ~= 'table' then return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'}) end",
    '    if not stream_appendable(keys[4]) or not stream_appendable(keys[5]) then',
    "      return cjson.encode({status='error', code='REDIS_STATE_INVALID'})",
    '    end',
    '  end',
    "  redis.call('HSET', keys[1], 'id', goal.id, 'json', args[1])",
    "  if expected == 0 then redis.call('ZADD', keys[2], created_ms, goal.id) end",
    "  if retrospective_ms then redis.call('ZADD', keys[3], retrospective_ms, goal.id) end",
    "  local result = {status='written', json=args[1]}",
    '  if has_event then',
    '    local streams = control_append_event(keys[4], keys[5], args[2])',
    '    result.globalStreamId = streams.globalStreamId',
    '    result.projectStreamId = streams.projectStreamId',
    '  end',
    '  return cjson.encode(result)',
    'end',
    'local function task_write(keys, args)',
    '  if #keys ~= 6 or #args ~= 5 then',
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  if not type_is(keys[1], 'hash') or not type_is(keys[2], 'zset') or not type_is(keys[3], 'hash') or not type_is(keys[4], 'zset') or not type_is(keys[5], 'stream') or not type_is(keys[6], 'stream') then",
    "    return cjson.encode({status='error', code='REDIS_STATE_INVALID'})",
    '  end',
    '  local task_ok, task = pcall(cjson.decode, args[1])',
    '  local expected = tonumber(args[3])',
    '  local created_ms = tonumber(args[4])',
    '  if not task_ok or not expected or not created_ms then',
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  local failure, stored, stored_json = cas_check(keys[1], expected, task, 'id')",
    "  if failure == 'VERSION_CONFLICT' then return cjson.encode({status='version_conflict', json=stored_json}) end",
    "  if failure then return cjson.encode({status='error', code=failure}) end",
    "  local has_event = args[2] ~= ''",
    '  if has_event then',
    ...
    '    end',
    '  end',
    "  redis.call('HSET', keys[1], 'id', task.id, 'json', args[1])",
    '  if expected == 0 then',
    "    redis.call('ZADD', keys[2], created_ms, task.id)",
    "    redis.call('ZADD', keys[4], created_ms, task.id)",
    '  end',
    "  if args[5] ~= '' then redis.call('HSET', keys[3], task.id, args[5]) else redis.call('HDEL', keys[3], task.id) end",
    "  local result = {status='written', json=args[1]}",
    '  if has_event then',
    '    local streams = control_append_event(keys[5], keys[6], args[2])',
    '    result.globalStreamId = streams.globalStreamId',
    ...
    '  return cjson.encode(result)',
    'end',
    // The one dispatch step that has to be atomic: two coordinators racing for
    // the last in-flight slot, the last dispatch of the hour, or the same path
    // must produce one dispatch and one refusal. The limits arrive as arguments
    // so the Function carries no policy constant.
    'local function task_dispatch(keys, args)',
    '  if #keys ~= 3 or #args ~= 8 then',
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  if not type_is(keys[1], 'hash') or not type_is(keys[2], 'hash') or not type_is(keys[3], 'zset') then",
    "    return cjson.encode({status='error', code='REDIS_STATE_INVALID'})",
    '  end',
    '  local task_ok, task = pcall(cjson.decode, args[1])',
    '  local expected = tonumber(args[2])',
    '  local now_ms = tonumber(args[3])',
    '  local window_ms = tonumber(args[4])',
    '  local max_in_flight = tonumber(args[5])',
    '  local max_per_hour = tonumber(args[6])',
    '  local paths_ok, match_paths = pcall(cjson.decode, args[7])',
    "  if not task_ok or not expected or not now_ms or not window_ms or not max_in_flight or not max_per_hour or not paths_ok or type(match_paths) ~= 'table' or args[8] == '' then",
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  local failure, stored, stored_json = cas_check(keys[1], expected, task, 'id')",
    "  if failure == 'VERSION_CONFLICT' then return cjson.encode({status='version_conflict', json=stored_json}) end",
    "  if failure then return cjson.encode({status='error', code=failure}) end",
    "  if stored == nil or (stored.state ~= 'ready' and stored.state ~= 'approved') then",
    "    return cjson.encode({status='version_conflict', json=stored_json})",
    '  end',
    "  local active = redis.call('HGETALL', keys[2])",
    '  local in_flight = 0',
    '  for index = 2, #active, 2 do',
    '    local other_ok, other = pcall(cjson.decode, active[index])',
    "    if other_ok and type(other) == 'table' then",
    '      in_flight = in_flight + 1',
    "      if type(other.matchPaths) == 'table' then",
    '        for _, mine in ipairs(match_paths) do',
    '          for _, theirs in ipairs(other.matchPaths) do',
    "            if type(mine) == 'string' and type(theirs) == 'string' and lease_overlaps(mine, theirs) then",
    "              return cjson.encode({status='denied', reason='path_overlap', detail=other.taskId or active[index - 1]})",
    '            end',
    '          end',
    '        end',
    '      end',
    ...
    "    return cjson.encode({status='denied', reason='in_flight_limit', detail=tostring(in_flight)})",
    '  end',
    "  local recent = redis.call('ZCOUNT', keys[3], now_ms - window_ms, '+inf')",
    '  if tonumber(recent) >= max_per_hour then',
    "    return cjson.encode({status='denied', reason='rate_limit', detail=tostring(recent)})",
    '  end',
    "  redis.call('HSET', keys[1], 'id', task.id, 'json', args[1])",
    "  redis.call('HSET', keys[2], task.id, args[8])",
    "  redis.call('ZADD', keys[3], now_ms, task.id)",
    "  return cjson.encode({status='dispatching', json=args[1]})",
    'end',
    // A wake-up for a coordinator: one XADD into its own inbox stream, which
    // must already exist with its consumer group (session_register creates
    // both), plus the event. It carries no work; the claim path acknowledges
    // it on delivery.
    'local function inbox_notice(keys, args)',
    '  if #keys ~= 3 or #args ~= 2 then',
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    "  if key_type(keys[1]) ~= 'stream' then return cjson.encode({status='error', code='INBOX_MISSING'}) end",
    "  if not type_is(keys[2], 'stream') or not type_is(keys[3], 'stream') then",
    "    return cjson.encode({status='error', code='REDIS_STATE_INVALID'})",
    '  end',
    '  local notice_ok, notice = pcall(cjson.decode, args[1])',
    '  local event_ok, event = pcall(cjson.decode, args[2])',
    "  if not notice_ok or type(notice) ~= 'table' or not event_ok or type(event) ~= 'table' then",
    "    return cjson.encode({status='error', code='REDIS_ARGUMENT_INVALID'})",
    '  end',
    '  if not stream_appendable(keys[1]) or not stream_appendable(keys[2]) or not stream_appendable(keys[3]) then',
    "    return cjson.encode({status='error', code='REDIS_STATE_INVALID'})",
    '  end',
    "  local stream_id = redis.call('XADD', keys[1], '*', 'item', args[1])",
    '  local streams = control_append_event(keys[2], keys[3], args[2])',
    "  return cjson.encode({status='queued', streamId=stream_id, globalStreamId=streams.globalStreamId, projectStreamId=streams.projectStreamId})",
    'end',
    ...
    register(registry.functions.autopilotPut, 'autopilot_put'),
    register(registry.functions.goalWrite, 'goal_write'),
    register(registry.functions.taskWrite, 'task_write'),
    register(registry.functions.taskDispatch, 'task_dispatch'),
    register(registry.functions.inboxNotice, 'inbox_notice'),
