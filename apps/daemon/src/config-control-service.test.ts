...
describe('config control service', () => {
  it('plans, approves, applies, detects drift, and safely rolls back native config', async () => {
    ...
    expect(await readFile(target, 'utf8')).toBe('model = "old"\napproval_policy = "on-request"\n');
    ...
    expect(appliedStates.indexOf('writing')).toBeLessThan(appliedStates.indexOf('files_committed'));
