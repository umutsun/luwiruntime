...
| `codex mcp list` shows `luwi-runtime` enabled, but a Codex task has no `luwi_*` tools | the LUWI hooks in `~/.codex/hooks.json` are not trust-approved (no `trusted_hash` under `[hooks.state]` in `~/.codex/config.toml`), so Codex skips them -> no attach record -> launcher exits `MCP_LAUNCH_FAILED` | approve the hooks in Codex (fresh session in the project folder, accept the hook-trust prompt), or use the headless bridge |
